...
async function buildTunnel() {
  ...
  const cached = Store.get('apt_forecast_' + cycle, null);
  if (cached) {
  ...
  const confidence = clip(25 + 65 * qualityScore - 10 * criticality, 20, 95);
  const forces = { spot_pressure: spotPressure, perp_pressure: perpPressure, positioning, liquidity, market_driver: driver };
  const supportingForces = Object.values(forces).filter(v => (v > 0 && direction === 'LONG') || (v < 0 && direction === 'SHORT')).length;
  const opposingForces = Object.values(forces).filter(v => (v < 0 && direction === 'LONG') || (v > 0 && direction === 'SHORT')).length;

  const forecast = {
    direction, confidence, originPrice: livePrice, invalidation: anchor,
    ...
    forces, supportingForces, opposingForces, tunnel: points, cycle,
  };
  hist.push({ physics_force_raw: rawForce, oi, cycle });
  Store.set('apt_states_' + SYM, hist.slice(-300));
  Store.set('apt_forecast_' + cycle, forecast);
