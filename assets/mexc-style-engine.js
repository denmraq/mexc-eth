...
//
// BUGFIX (2026-09-14): Forex Factory rate-limits this endpoint to ~2
...
// events this week" in the UI. That's very likely why a real FOMC
// meeting inside the 48h window never showed up: the fetch was probably
// failing quietly, not correctly returning zero events. Two changes:
//   1. Cache TTL raised from 30min to 6h — the calendar itself doesn't
//      change that often, and this keeps normal usage far under FF's
//      rate limit instead of skating right against it.
//   2. fetchCalendarRaw() now returns a status ('ok' | 'stale_cache' |
//      'failed') alongside the data, and upcomingEvents()/the estimate
//      object propagate it, so the UI can say "calendar unavailable"
